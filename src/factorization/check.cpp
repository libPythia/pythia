...
auto check_graph_integrity(Grammar const & g) -> bool {
    ...
    for (auto const nonterminal : g.nonterminals.in_use_nonterminals()) {
        ...
        if (auto const last = nonterminal->last;
            last->maps_to->occurrences_without_successor.find(last) ==
            last->maps_to->occurrences_without_successor.end()) {
            std::cerr << "\nMissing occurrence for last symbol of a rule";
            ok = false;
        }
        ...
        while (!is_last(n)) {
            ...
            auto const it = n->maps_to->occurrences_with_successor.find(next->maps_to);
            if (it == n->maps_to->occurrences_with_successor.end()) {
        ...
        // occurrences
        for (auto const & [next, occurrence] : nonterminal->occurrences_with_successor) {
            if (occurrence->maps_to != nonterminal) {
                std::cerr << "\nOccurence is registered in wront non-terminal";
                ok = false;
            } else if (is_last(occurrence)) {
                std::cerr << "\nOccurence without successor is registered with a successor";
                ok = false;
            } else if (next_node(occurrence)->maps_to != next) {
                std::cerr << "\nOccurence with successor is registered with wrong successor";
                ok = false;
            }
        }

        for (auto const & occurrence : nonterminal->occurrences_without_successor) {
            if (occurrence->maps_to != nonterminal) {
                std::cerr << "\nOccurence is registered in wront non-terminal";
                ok = false;
            } else if (!is_last(occurrence)) {
...
static auto is_root(NonTerminal const * nt) -> bool {
    return nt->occurrences_with_successor.size() + nt->occurrences_without_successor.size() == 0;
}
...
auto check_grammar_constraints(Grammar const & g) -> bool {
    ...
    for (auto const nonterminal : g.nonterminals.in_use_nonterminals()) {
        ...
        if (!is_root(nonterminal)) {
            ...
            auto count = 0u;
            for (auto const & it : nonterminal->occurrences_with_successor)
                count += it.second->repeats;
            for (auto const & n : nonterminal->occurrences_without_successor)
                count += n->repeats;
