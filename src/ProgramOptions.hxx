...
#    define PROGRAMOPTIONS_ASSERT(Expression, Message)                                             \
        do {                                                                                       \
            if (!(Expression))                                                                     \
                throw std::logic_error(("ProgramOptions.hxx:" + std::to_string(__LINE__) + ": ") + \
                                       (Message));                                                 \
        } while (0)
...
#if !defined(PROGRAMOPTIONS_NO_COLORS) && !defined(PROGRAMOPTIONS_WINDOWS) && \
      !defined(PROGRAMOPTIONS_ANSI)
...
#    endif  // PROGRAMOPTIONS_ANSI
    ...
    operator std::ostream &() const { return m_stream; }
...
inline color_resetter operator<<(std::ostream & stream, color_t color) { return { stream, color }; }
...
struct error_t {
    friend std::ostream & operator<<(std::ostream & stream,
                                     error_t const & /* object */) {  // -Wunused-parameter
        return stream << red << "error: ";
    }
};
inline error_t error() { return {}; }
...
inline suggestion_t suggest(std::string const & what) { return { what }; }

template <typename arg_t> struct ignore_t {
    arg_t const & arg;
...
template <typename arg_t> ignore_t<arg_t> ignoring(arg_t const & arg) { return { arg }; }

// Compatibility stuff for the lack of C++14 support
template <typename T, typename... args_t> std::unique_ptr<T> make_unique(args_t &&... args) {
...
namespace detail {
    ...
    template <typename T,
              T i,
              typename = typename make_integer_sequence_impl<T, i / 2>::type,
              typename = typename make_integer_sequence_impl<T, i % 2>::type>
    ...
    template <typename T, T i> struct make_integer_sequence_impl<T, i, true, false> {
        using type = integer_sequence<T>;
    };
    template <typename T, T i> struct make_integer_sequence_impl<T, i, false, true> {
        using type = integer_sequence<T, 0>;
    };
    template <typename T, T i> struct make_integer_sequence_impl<T, i, false, false> {
...
inline std::size_t damerau_levenshtein(char const * a,
                                       char const * b,
                                       std::size_t i,
                                       std::size_t j,
                                       std::size_t cutoff = std::numeric_limits<std::size_t>::max(),
    ...
    if (j == 0)
        return i;
    std::size_t result =
          std::min(std::min(damerau_levenshtein(a, b, i - 1, j, cutoff, distance + 1),
                   ...
                   damerau_levenshtein(a,
                                       ...
                                       j - 1,
                                       cutoff,
                                       distance + !case_insensitive_eq(a[i - 1], b[j - 1])));
    if (i >= 2 && j >= 2 && case_insensitive_eq(a[i - 1], b[j - 2]) &&
        case_insensitive_eq(a[i - 2], b[j - 1]))
...
inline std::size_t damerau_levenshtein(std::string a,
                                       std::string b,
                                       std::size_t cutoff = std::numeric_limits<std::size_t>::max()) {
...
template <typename enum_t> typename std::underlying_type<enum_t>::type enum2int(enum_t value) {
    return static_cast<typename std::underlying_type<enum_t>::type>(value);
}

enum value_type { void_, string, i32, i64, u32, u64, f32, f64 };
static char const * value_type_strings[] = { "void", "string", "i32", "i64",
                                             "u32",  "u64",    "f32", "f64" };
inline char const * vt2str(value_type type) { return value_type_strings[enum2int(type)]; }
...
namespace detail {
    template <value_type type> struct vt2type_impl {};
    template <> struct vt2type_impl<void_> { using type = void_t; };
    template <> struct vt2type_impl<string> { using type = string_t; };
    template <> struct vt2type_impl<i32> { using type = i32_t; };
    template <> struct vt2type_impl<i64> { using type = i64_t; };
    template <> struct vt2type_impl<u32> { using type = u32_t; };
    template <> struct vt2type_impl<u64> { using type = u64_t; };
    template <> struct vt2type_impl<f32> { using type = f32_t; };
    template <> struct vt2type_impl<f64> { using type = f64_t; };
}  // namespace detail
template <value_type type> using vt2type = typename detail::vt2type_impl<type>::type;
...
namespace detail {
    ...
    template <> struct type2vt_impl<void, false, false, false> {
        static constexpr value_type value = void_;
    };
    template <> struct type2vt_impl<std::string, false, false, false> {
        static constexpr value_type value = string;
    };
    template <typename T> struct type2vt_impl<T, true, false, true> {
        static constexpr std::size_t T_bits = sizeof(T) * std::numeric_limits<unsigned char>::digits;
        static_assert(T_bits == 32 || T_bits == 64,
                      "type2vt: only 32 or 64 bit wide signed integral types supported");
        static constexpr value_type value = (T_bits == 32) ? i32 : i64;
    };
    template <typename T> struct type2vt_impl<T, true, false, false> {
        static constexpr std::size_t T_bits = sizeof(T) * std::numeric_limits<unsigned char>::digits;
        static_assert(T_bits == 32 || T_bits == 64,
                      "type2vt: only 32 or 64 bit wide unsigned integral types supported");
        static constexpr value_type value = (T_bits == 32) ? u32 : u64;
    };
    template <typename T> struct type2vt_impl<T, false, true, true> {
        static constexpr std::size_t T_bits = sizeof(T) * std::numeric_limits<unsigned char>::digits;
        static_assert(T_bits == 32 || T_bits == 64,
                      "type2vt: only 32 or 64 bit wide floating point types supported");
        static constexpr value_type value = (T_bits == 32) ? f32 : f64;
    };
}  // namespace detail
template <typename T> struct type2vt : detail::type2vt_impl<typename std::remove_cv<T>::type> {};

template <typename T> T pow(T base, unsigned exp) {
    if (exp == 0)
        return 1;
...
template <typename T> T pow(T base, int exp) {
...
template <typename T> struct pow10<T, 0> : public std::integral_constant<T, 1> {};

template <unsigned i, bool = (i < 10)>
struct log10 : public std::integral_constant<unsigned, 1 + log10<i / 10>::value> {};
template <> struct log10<0, true>;
template <unsigned i> struct log10<i, true> : public std::integral_constant<unsigned, 0> {};
...
template <typename T> class parsing_report {
  ...
  public:
    ...
    bool good() const { return error == error_code::none; }
...
namespace detail {
    ...
    bool expect_impl(forward_iterator_t & first, forward_iterator_t last, char head, tail_t const &... tail) {
        return first != last && case_insensitive_eq(*first, head) &&
               expect_impl(++first, last, tail...);
    }
}  // namespace detail
...
namespace detail {
    ...
    bool expect_unpacker(forward_iterator_t & first,
                         forward_iterator_t last,
                         const char (&args)[N],
                         integer_sequence<std::size_t, indices...>) {
...
parsing_report<T> str2uint(forward_iterator_t first, forward_iterator_t last) {
    static_assert(std::numeric_limits<T>::is_integer && !std::numeric_limits<T>::is_signed,
                  "str2uint only supports unsigned integral types");
    ...
    if (expect(first, last, 'x')) {
        ...
        for (; first != last && *first == '0'; ++first)
            ;
        std::size_t digits = 0;
        enum : std::size_t { max_digits = std::numeric_limits<T>::digits / 4 };
        for (int d {}; first != last && digits <= max_digits && (d = get_hex_digit(*first)) >= 0;
             ++first, ++digits)
            (result <<= 4) |= d;
        ...
        for (; first != last && *first == '0'; ++first)
            ;
        std::size_t digits = 0;
        enum : std::size_t { max_digits = std::numeric_limits<T>::digits };
        for (int d {}; first != last && digits <= max_digits && (d = get_bin_digit(*first)) >= 0;
             ++first, ++digits)
            (result <<= 1) |= d;
        if (digits > max_digits)
            return error_code::out_of_range;
        ...
        enum : std::size_t { max_decimals = 1 + std::numeric_limits<T>::digits10 };
        for (int d {}; first != last && decimals <= max_decimals && (d = get_digit(*first)) >= 0;
             ++first, ++decimals)
...
template <typename T> parsing_report<T> str2uint(std::string const & str) {
    return str2uint<T>(str.begin(), str.end());
}
template <typename T> parsing_report<T> str2uint(char const * str) {
...
parsing_report<T> str2int(forward_iterator_t first, forward_iterator_t last) {
    static_assert(std::numeric_limits<T>::is_integer && std::numeric_limits<T>::is_signed,
                  "str2uint only supports signed integral types");
    static_assert(std::numeric_limits<T>::min() == -std::numeric_limits<T>::max() - 1,
                  "type insufficient; doesn't employ two's complement");

    using unsigned_t = typename std::make_unsigned<T>::type;
    static_assert(std::numeric_limits<unsigned_t>::max() > std::numeric_limits<T>::max(),
                  "type insufficient");
...
template <typename T> parsing_report<T> str2int(std::string const & str) {
    return str2int<T>(str.begin(), str.end());
}
template <typename T> parsing_report<T> str2int(char const * str) {
...
parsing_report<T> str2flt(forward_iterator_t first, forward_iterator_t last) {
    static_assert(std::is_floating_point<T>::value,
                  "str2flt only supports built-in floating point types");
    ...
    static_assert(std::numeric_limits<T>::has_quiet_NaN,
                  "type insufficient; doesn't support quiet NaNs");
    static_assert(std::numeric_limits<T>::has_infinity,
                  "type insufficient; doesn't support infinities");
    using std::isinf;
    ...
    } else if (expect(first, last, "infinity") || expect(first, last, "inf")) {
...
template <typename T> parsing_report<T> str2flt(std::string const & str) {
    return str2flt<T>(str.begin(), str.end());
}
template <typename T> parsing_report<T> str2flt(char const * str) {
    return str2flt<T>(str, str + std::strlen(str));
}

template <typename T> std::string int2str(T const & value) {
    static_assert(std::numeric_limits<T>::is_specialized && std::numeric_limits<T>::is_integer,
                  "int2str only supports integral types");

    return std::to_string(value);
}
template <typename T> std::string flt2str(T const & value) {
    static_assert(std::numeric_limits<T>::is_specialized && !std::numeric_limits<T>::is_integer,
                  "flt2str only supports floating point types");
...
struct value_vector_base {
    ...
    value const * end() const { return begin() + size(); }
    value * end() {
        return const_cast<value *>(static_cast<value_vector_base const &>(*this).end());
    }

    value const & get() const {
        PROGRAMOPTIONS_ASSERT(begin() != nullptr,
                              "value_vector_base: cannot access elements of a void_ vector");
        PROGRAMOPTIONS_ASSERT(size() != 0,
                              "value_vector_base: cannot access elements of an empty vector");
        return begin()[size() - 1];
    }
    value & get() {
...
template <bool is_void, bool is_single> class value_vector;
template <> class value_vector<false, false> final : public value_vector_base {
    std::vector<value> m_values;

  public:
    virtual std::size_t size() const override { return m_values.size(); }
    virtual value const * begin() const override { return &*m_values.begin(); }
    virtual void push_back(value const & object) override { m_values.push_back(object); }
};
template <> class value_vector<false, true> final : public value_vector_base {
    bool m_set = false;
    value m_value;

  public:
    virtual std::size_t size() const override { return m_set ? 1 : 0; }
    virtual value const * begin() const override { return &m_value; }
    virtual void push_back(value const & object) override {
        m_value = object;
        m_set = true;
    }
};
template <> class value_vector<true, false> final : public value_vector_base {
    std::size_t m_counter = 0;

  public:
    virtual std::size_t size() const override { return m_counter; }
    virtual value const * begin() const override { return nullptr; }
    virtual void push_back(value const & /* object */) override {  // -Wunused-parameter
        ++m_counter;
    }
};
template <> class value_vector<true, true> final : public value_vector_base {
    bool m_set = false;
...
namespace detail {
    template <typename fun_t, typename... args_t> class is_invocable_sfinae {
        template <typename _fun_t, typename... _args_t> static std::false_type test(...);
...
template <typename invocable_t> class callback_storage : public callback_base {
  protected:
...
#ifdef PROGRAMOPTIONS_DEBUG
    virtual bool good() const override { return false; }
#endif  // PROGRAMOPTIONS_DEBUG
};
template <typename invocable_t, bool with_string, bool with_type>
struct callback<void_, invocable_t, true, with_string, with_type>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & /* object */) override {  // -Wunused-parameter
        this->m_invocable();
    }
...
struct callback<string, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.string); }
};
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<i32, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.i32); }
};
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<i64, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.i64); }
};
template <typename invocable_t, bool with_void>
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<u32, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.u32); }
};
template <typename invocable_t, bool with_void>
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<u64, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.u64); }
};
template <typename invocable_t, bool with_void>
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<f32, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.f32); }
};
template <typename invocable_t, bool with_void>
...
template <typename invocable_t, bool with_void, bool with_string>
struct callback<f64, invocable_t, with_void, with_string, true>
      : public callback_storage<invocable_t> {
    using callback_storage<invocable_t>::callback_storage;
    virtual void invoke(value const & object) override { this->m_invocable(object.f64); }
};
template <typename invocable_t, bool with_void>
...
template <value_type type, typename underlying_t> class value_iterator {
    static_assert(type != void_, "iterating over voids disallowed");
    static_assert(std::is_same<typename std::iterator_traits<underlying_t>::iterator_category,
                               std::random_access_iterator_tag>::value,
                  "underlying iterator incompatible");
  ...
  public:
    ...
    reference operator*() const { return deref(std::integral_constant<::po::value_type, type> {}); }
    pointer operator->() const { return &operator*(); }

    bool operator==(value_iterator const & rhs) const { return m_underlying == rhs.m_underlying; }
    bool operator!=(value_iterator const & rhs) const { return m_underlying != rhs.m_underlying; }
    bool operator<(value_iterator const & rhs) const { return m_underlying < rhs.m_underlying; }
    bool operator<=(value_iterator const & rhs) const { return m_underlying <= rhs.m_underlying; }
    bool operator>(value_iterator const & rhs) const { return m_underlying > rhs.m_underlying; }
...
value_iterator<type, underlying_t>
operator+(value_iterator<type, underlying_t> object,
          typename value_iterator<type, underlying_t>::difference_type n) {
    return object += n;
}
template <value_type type, typename underlying_t>
value_iterator<type, underlying_t> operator+(typename value_iterator<type, underlying_t>::difference_type n,
                                             value_iterator<type, underlying_t> object) {
    return object += n;
}
template <value_type type, typename underlying_t>
value_iterator<type, underlying_t>
...
namespace detail {
    template <typename container_t, typename... args_t> class has_push_back_sfinae {
        template <typename _container_t, typename... _args_t> static std::false_type test(...);
        template <typename _container_t, typename... _args_t>
        static std::true_type
        test(decltype(std::declval<_container_t>().push_back(std::declval<_args_t>()...)) *);
...
namespace detail {
    template <typename container_t, typename... args_t> class has_push_sfinae {
        template <typename _container_t, typename... _args_t> static std::false_type test(...);
        template <typename _container_t, typename... _args_t>
        static std::true_type
        test(decltype(std::declval<_container_t>().push(std::declval<_args_t>()...)) *);
...
class option {
  ...
  public:
    void make_immutable() { m_mutable = false; }
  ...
  private:
    ...
    value_vector_base const & get_vector() const {
        PROGRAMOPTIONS_ASSERT(available(),
    ...
    template <typename T>
    parsing_report<value>
    make_value(T const & integer,
        ...
        switch (get_type()) {
            ...
            case u64:
                ...
                break;
            case f32: result.f32 = static_cast<f32_t>(integer); break;
            case f64: result.f64 = static_cast<f64_t>(integer); break;
            default: return error_code::conversion_error;
        }
        if (out_of_range)
            return error_code::out_of_range;
    ...
    template <typename T>
    parsing_report<value>
    make_value(T const & floating,
               typename std::enable_if<std::is_floating_point<T>::value>::type * = nullptr) const {
        value result;
        switch (get_type()) {
            case string: break;
            case f32: result.f32 = static_cast<f32_t>(floating); break;
            case f64: result.f64 = static_cast<f64_t>(floating); break;
            default: return error_code::conversion_error;
        }
    ...
    template <value_type type> bool valid_iterator_type() const {
        return type == string || type == get_type();
    }
    template <value_type type> void assert_iterator_type() const {
  ...
  public:
    ...
    bool was_set() const { return m_data != nullptr; }
    bool available() const { return m_data != nullptr || m_fallback != nullptr; }
    ...
    template <value_type type> value_iterator<type, iterator> begin() const {
        assert_iterator_type<type>();
        return value_iterator<type, iterator>(begin());
    }
    template <value_type type> value_iterator<type, iterator> cbegin() const {
        return begin<type>();
    }
    template <value_type type>
    ...
    template <value_type type> value_iterator<type, iterator> end() const {
        assert_iterator_type<type>();
        return value_iterator<type, iterator>(end());
    }
    template <value_type type> value_iterator<type, iterator> cend() const { return end<type>(); }
    template <value_type type> std::reverse_iterator<value_iterator<type, iterator>> rend() const {
        return std::reverse_iterator<value_iterator<type, iterator>>(begin<type>());
    }
    template <value_type type> std::reverse_iterator<value_iterator<type, iterator>> crend() const {
        return rend<type>();
    }

    template <value_type type> std::vector<vt2type<type>> to_vector() const {
    ...
    option & no_abbreviation() { return abbreviation('\0'); }
    char get_abbreviation() const { return m_abbreviation; }

  private:
    static bool valid_description(std::string const & value) {
        return std::find_if_not(value.begin(), value.end(), [](char c) {
                   return std::isprint(c) || c == '\n';
               }) == value.end();
    }
    static void assert_description(std::string const & value) {
        PROGRAMOPTIONS_ASSERT(valid_description(value),
                              "description may only consist of printable characters and newlines");
  ...
  private:
    static bool valid_type(value_type type) {
        switch (type) {  // switch(enum) ensures that a warning is generated whenever an additional
                         // enum value is added
            case void_:
            case string:
            case i32:
  ...
  public:
    option & type(value_type type) {
        PROGRAMOPTIONS_ASSERT((m_type == type) || (m_fallback == nullptr && m_data == nullptr &&
                                                   m_callbacks.empty()),
    ...
    option & multi(bool make_multi) {
        PROGRAMOPTIONS_ASSERT((m_multi == make_multi) || (m_fallback == nullptr &&
                                                          m_data == nullptr && m_callbacks.empty()),
                              "multi() must be set prior to: fallback(), callback(), parsing");
        mutable_operation();
        m_multi = make_multi;
        return *this;
    }
    option & single(bool make_single) { return multi(!make_single); }
    option & multi() { return multi(true); }
    option & single() { return single(true); }
    bool is_multi() const { return m_multi; }
    bool is_single() const { return !is_multi(); }

  private:
    template <typename T> parsing_report<value> fallback_parse(T && arg) {
    ...
    void fallback_single(head_t && head, tail_t &&... /* tail */) {  // -Wunused-parameter
        PROGRAMOPTIONS_ASSERT(sizeof...(tail_t) == 0,
                              "fallback: too many arguments for single() option");
    ...
    template <typename... args_t> void fallback_multi(args_t &&... args) {
  ...
  public:
    template <typename... args_t> option & fallback(args_t &&... args) {
    ...
    template <typename invocable_t> option & callback(invocable_t && invocable) {
        mutable_operation();
        std::unique_ptr<callback_base> new_callback;
        switch (get_type()) {
            case void_:
                new_callback =
                      make_unique<::po::callback<void_, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case string:
                new_callback =
                      make_unique<::po::callback<string, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case i32:
                new_callback =
                      make_unique<::po::callback<i32, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case i64:
                new_callback =
                      make_unique<::po::callback<i64, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case u32:
                new_callback =
                      make_unique<::po::callback<u32, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case u64:
                new_callback =
                      make_unique<::po::callback<u64, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case f32:
                new_callback =
                      make_unique<::po::callback<f32, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                break;
            case f64:
                new_callback =
                      make_unique<::po::callback<f64, typename std::remove_reference<invocable_t>::type>>(
                            std::forward<invocable_t>(invocable));
                // 	break;
                // default:
                // 	assert(false);
    ...
    template <typename container_t>
    option & bind_container(container_t & container,
                            typename std::enable_if<has_push_back_vt<container_t>::value>::type * = nullptr) {
        using value_type = typename container_t::value_type;
        return bind_container(container, [](container_t & container, value_type const & value) {
            container.push_back(value);
        });
    }
    template <typename container_t>
    option & bind_container(container_t & container,
                            typename std::enable_if<has_push_vt<container_t>::value>::type * = nullptr) {
        using value_type = typename container_t::value_type;
        return bind_container(container, [](container_t & container, value_type const & value) {
    ...
    template <typename T> option & bind(T & target) {
        type(type2vt<T>::value);
        single();
        callback([&target](T const & x) { target = x; });
        return *this;
    }
    template <typename T> option & bind(std::vector<T> & target) { return bind_container(target); }
    template <typename T> option & bind(std::deque<T> & target) { return bind_container(target); }
    template <typename T> option & bind(std::list<T> & target) { return bind_container(target); }
    template <typename T> option & bind(std::stack<T> & target) { return bind_container(target); }
    template <typename T> option & bind(std::queue<T> & target) { return bind_container(target); }
    template <typename T> option & bind(std::priority_queue<T> & target) {
  ...
  public:
    template <typename... args_t> error_code parse(args_t &&... args) {
    ...
    void print_data(std::ostream & stream) const {
        ...
        switch (get_type()) {
            ...
            case string:
                ...
                break;
            case i32: printer = [](std::ostream & s, value const & v) { s << v.i32; }; break;
            case i64: printer = [](std::ostream & s, value const & v) { s << v.i64; }; break;
            case u32: printer = [](std::ostream & s, value const & v) { s << v.u32; }; break;
            case u64: printer = [](std::ostream & s, value const & v) { s << v.u64; }; break;
            case f32: printer = [](std::ostream & s, value const & v) { s << v.f32; }; break;
            case f64: printer = [](std::ostream & s, value const & v) { s << v.f64; }; break;
...
class parser {
  ...
  public:
    ...
    void silent() { m_output_destination = nullptr; }
    bool is_silent() const { return m_output_destination == nullptr; }
    void verbose(std::ostream & destination) { m_output_destination = &destination; }
    bool is_verbose() const { return !is_silent(); }

  private:
    template <typename arg_t> void error_nonoption_arguments(arg_t const & arg) {
        assert(is_verbose());
        *m_output_destination << error() << "non-option arguments not allowed" << ignoring(arg);
    }
    template <typename arg_t> void error_unrecognized_option(arg_t const & arg) {
        assert(is_verbose());
    ...
    void check_spelling(char const * long_option) {
        ...
        enum : std::size_t { distance_cutoff = 4 };
        ...
        for (auto iter = m_options.begin(); iter != m_options.end(); ++iter) {
            if (iter->first.empty())
                continue;
            const std::size_t distance = damerau_levenshtein(name_begin,
                                                             iter->first.c_str(),
                                                             length,
                                                             iter->first.length(),
                                                             distance_cutoff);
            ...
            *m_output_destination << "\' ";
            switch (code) {
                ...
                case error_code::conversion_error:
                    *m_output_destination << "expects an argument of type "
                                          << vt2str(option->second.get_type());
                    break;
    ...
    bool dashed_non_option(char * arg) {
        return arg[0] == '-' && (is_digit(arg[1]) || arg[1] == '.') &&
               find_abbreviation(arg[1]) == m_options.end();
    }
    bool extract_argument(options_t::iterator option,
                          std::size_t argc,
                          char ** argv,
                          std::size_t & i,
                          std::size_t j) {
        ...
        if (argv[i][j] == '\0') {
            // -v data
            if (option->second.get_type() != void_ && i + 1 < argc &&
            ...
            if (is_verbose())
                *m_output_destination
                      << error() << "unexpected character \'" << argv[i][j] << "\'"
                      << ignoring(argv[i])
  ...
  public:
    ...
    bool operator()(int int_argc, char ** argv) {
        ...
        PROGRAMOPTIONS_ASSERT(std::none_of(m_options.begin(),
                                           m_options.end(),
                                           [](options_t::value_type const & x) {
                                               return x.second.was_set();
                                           }),
                    ...
                    if (head == m_options.end()) {
                        ...
                        for (std::size_t j = 1; (c = argv[i][j]) != '\0'; ++j) {
                            if (!std::isprint(c) || c == '-') {
                                good = false;
                                if (is_verbose())
                                    *m_output_destination << error() << "invalid character \'" << c
                                                          << "\'" << ignoring(&argv[i][j]) << '\n';
                                break;
                            }
                            ...
                            if (opt->second.get_type() != void_) {
                                good = false;
                                if (is_verbose())
                                    *m_output_destination << error()
                                                          << "non-void options not allowed in "
                                                             "option packs"
                                                          << ignoring(c) << '\n';
                                continue;
                            }
    ...
    bool parse(int argc, char ** argv) { return operator()(argc, argv); }
  ...
  private:
    ...
    static bool valid_designator(std::string const & designator) {
        ...
        if (designator[0] == '-')
            return false;
        return std::find_if_not(designator.begin(), designator.end(), &valid_designator_character) ==
               designator.end();
    }
    option & operator_brackets_helper(std::string && designator) {
        PROGRAMOPTIONS_ASSERT(valid_designator(designator),
                              "operator[]: designator may only consist of letters, hyphens and "
                              "underscores and mustn't start with a hyphen");
        ...
        if (result.second) {
            ...
            if (empty)
                result.first->second.type(string).multi();
        }
    ...
    void print_help(std::ostream & stream) const {
        ...
        for (auto iter = m_order.begin(); iter != m_order.end(); ++iter) {
            ...
            if (verbose) {
                ...
                const int rem =
                      static_cast<int>(verbose_width) - 2 - static_cast<int>(opt.first.size());
                if (rem >= 0)
